...
import { AaveV1EtherAdapter, IERC20 } from '../../typechain'
...
describe('AaveV1EtherAdapter', () => {
  ...
  let implementation: AaveV1EtherAdapter
  let adapter: AaveV1EtherAdapter
  let token: IERC20
  let aToken: IERC20
  let amountMinted: BigNumber

  before('Deploy implementation', async () => {
    implementation = await deployContract('AaveV1EtherAdapter', '0x24a42fD28C976A61Df5D00D0599C34c4f90748c8')
    token = await getContract('0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', 'IERC20')
    aToken = await getContract('0x3a3a65aab0dd2a17e3f1947ba16138cd37d08c04', 'IERC20')
    adapter = await deployClone(implementation, 'AaveV1EtherAdapter');
  ...
  describe('settings', () => {
    it('name()', async () => {
      expect(await adapter.name()).to.eq(`Aave V1 ETH Adapter`)
    })
